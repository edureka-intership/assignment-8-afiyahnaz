...
let [userDetails, setUserDetails] = useState( getTOkenDetails);
  ...
  let getRestaurantDetails = async () => {
    try {
      ...
      let URL = 'http://localhost:5003/api/getRestaurantDetailsById/' + id;
  ...
  let getMenuItems = async () => {
    try {
      ...
      let URL = `http://localhost:5003/api/getMenuItemListByRestaurantId/${id}`;
      let { data } = await axios.get(URL);
      if (data.status === true) {
  ...
  let displayRazorpay = async () => {
    ...
    var { data } = await axios.post(
      ...
      "http://localhost:5003/api/payment/genOrder",
      serverData
    );
    console.log(data);
    var order = data.order;
    var options = {
      // key: "rzp_test_KvxHzp17jmYyxR",
